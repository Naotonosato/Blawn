...
class SharedFromThis {
    ...
    void* operator new(size_t, Args&&...) {
        ...
        static_assert(operator_new_is_not_used,
                      "Class that inherits from SharedFromThis cannot be "
                      "instantiated with operator new.");
    }

    template <typename... Args>
    static std::shared_ptr<T> create(Args&&... args) {
        constexpr bool is_constructor_is_callable =
            std::is_constructible<T, Args&&...>::value;
        static_assert(!is_constructor_is_callable,
                      "If the class inherits from SharedFromThis,the class of "
                      "constructor must be protected.");

        auto shared_this =
            std::make_shared<CreateHelper>(std::forward<Args>(args)...);
        return shared_this;
    }
};
}  // namespace behaviors