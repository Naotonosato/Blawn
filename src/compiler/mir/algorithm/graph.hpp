...
template <typename VertexType> using Edge = typename std::pair<VertexType,VertexType>;
template <typename VertexType> using Edges = typename std::unordered_map<VertexType,std::vector<Edge<VertexType>>>;
...
namespace directed_pseuduoforest
{
    ...
    class DirectedPseudoForest
    {
        ...
        std::vector<Edge<VertexType>> const& get_edges() const
        {
            return edges;
        }
...
class Node : public std::enable_shared_from_this<NodeType> {
    ...
    std::vector<std::shared_ptr<NodeType>> observers;//self instance has influence for observers;
   // Node()=delete;
