...
namespace algorithm
{
    ...
    {
        /*
        This algorithm may be possible to improve. Becaouse container of root nodes and
        ranks are unordered_map not fast accessing.
        To replace unordered_map to vector is way to optimize speed.
        But for simplicity,not yet implemented.
        */
        private:
        ...
        void add_node(NodeType node,NodeType parent_node)
        {
            ...
            if (ranks.count(root_of_node)) ranks[root_of_node] += 1;
            else ranks[root_of_node] = 0;
        }
