...
ASTGenerator::ASTGenerator(llvm::Module& module, llvm::IRBuilder<>& ir_builder,
    ...
    : module(module),
      ...
      no_value_node(new ast::Node(0, {}, ir_generators.ir_generator)),
      logger() {
    // into_namespace("main");
}

void ASTGenerator::generate(std::vector<std::shared_ptr<ast::Node>> program) {
...
std::shared_ptr<ast::Node> ASTGenerator::create_assign(
    std::string name, std::shared_ptr<ast::Node> right_node, bool is_global) {
    if (variable_collector.exist(name)) {
        auto variable = variable_collector.get(name);
        auto assigment =
            std::shared_ptr<ast::AssignmentNode>(new ast::AssignmentNode(
    ...
    if (argument_collector.exist(name)) {
        ...
        auto variable =
            std::shared_ptr<ast::VariableNode>(new ast::VariableNode(
...
std::shared_ptr<ast::Node> ASTGenerator::create_assign(
    std::shared_ptr<ast::AccessNode> left, std::shared_ptr<ast::Node> right) {
    auto assigment = std::shared_ptr<ast::AssignmentNode>(
        new ast::AssignmentNode(line_number, current_scope,
                               ir_generators.assigment_generator, right, left));
    return assigment;
}

std::shared_ptr<ast::Node> ASTGenerator::create_store(
    std::shared_ptr<ast::Node> pointer, std::shared_ptr<ast::Node> object) {
    auto node = std::shared_ptr<ast::DeepCopyNode>(
        new ast::DeepCopyNode(line_number, current_scope,
                           ir_generators.store_generator, pointer, object));
    return node;
}

std::shared_ptr<ast::Node> ASTGenerator::declare_C_function(
    std::string name, std::vector<std::shared_ptr<ast::Node>> args,
    std::shared_ptr<ast::Node> return_) {
    std::shared_ptr<ast::DeclareCNode> node(new ast::DeclareCNode(
        line_number, current_scope, ir_generators.declare_C_generator, name,
        args, return_));
    return std::move(node);
}

std::shared_ptr<ast::Node> ASTGenerator::get_named_value(std::string name) {
    if (variable_collector.exist(name)) {
...
std::shared_ptr<ast::Node> ASTGenerator::create_C_member(std::string name) {
    std::string splitter =
        "__PTR__"
        " ";
    ...
    auto node = std::shared_ptr<ast::Node>(
        new ast::NullNode(line_number, current_scope,
                          ir_generators.null_generator, name, class_node));
    return node;
}
...
void ASTGenerator::add_argument(std::string arg_name) {
    ...
    arg_scope.pop_back();
    auto argument = std::shared_ptr<ast::ArgumentNode>(new ast::ArgumentNode(
...
std::shared_ptr<ast::FunctionNode> ASTGenerator::add_function(
    ...
    auto func = std::shared_ptr<ast::FunctionNode>(new ast::FunctionNode(
...
std::shared_ptr<ast::ClassNode> ASTGenerator::create_class(
    std::string name, std::vector<std::string> arguments,
    std::vector<std::shared_ptr<ast::Node>> members_definition,
    std::vector<std::shared_ptr<ast::FunctionNode>> methods) {
    if (class_collector.exist(name)) {
        logger.set_line_number(line_number);
        logger.multiple_definition_error("class", name);
    }
    auto class_ = std::shared_ptr<ast::ClassNode>(new ast::ClassNode(
...
std::shared_ptr<ast::ClassNode> ASTGenerator::create_C_type(
    std::string name,
    std::vector<std::shared_ptr<ast::Node>> members_definition) {
    /*
    ...
    */
    auto c_type = std::shared_ptr<ast::ClassNode>(new ast::ClassNode(
...
std::shared_ptr<ast::Node> ASTGenerator::create_call(
    ...
    if (get_blawn_context().exist_builtin_function(name)) {
        ...
        auto calling =
            std::shared_ptr<ast::CallFunctionNode>(new ast::CallFunctionNode(
    ...
    if (class_collector.exist(name)) {
        ...
        auto constructor = std::shared_ptr<ast::CallConstructorNode>(
            new ast::CallConstructorNode(
...
std::shared_ptr<ast::IntegerNode> ASTGenerator::create_integer(int num) {
    auto Integer = std::shared_ptr<ast::IntegerNode>(new ast::IntegerNode(
        line_number, current_scope, ir_generators.int_ir_generator));
    Integer->int_num = num;
    return std::move(Integer);
}

std::shared_ptr<ast::FloatNode> ASTGenerator::create_float(double num) {
    auto float_ = std::shared_ptr<ast::FloatNode>(new ast::FloatNode(
        line_number, current_scope, ir_generators.float_ir_generator));
    float_->float_num = num;
    return std::move(float_);
}

std::shared_ptr<ast::Node> ASTGenerator::create_minus(
    std::shared_ptr<ast::Node> expr) {
    auto zero = std::shared_ptr<ast::Node>(create_integer(0));
    auto minus = attach_operator(zero, expr, "-");
    return minus;
}

std::shared_ptr<ast::StringNode> ASTGenerator::create_string(std::string str) {
    auto string = std::make_shared<ast::StringNode>(
        line_number, current_scope, ir_generators.string_generator);
    string->string = str;
    return string;
}

std::shared_ptr<ast::BinaryExpressionNode> ASTGenerator::attach_operator(
    std::shared_ptr<ast::Node> left_node, std::shared_ptr<ast::Node> right_node,
    const std::string operator_kind) {
    auto expression = std::shared_ptr<ast::BinaryExpressionNode>(
        new ast::BinaryExpressionNode(
            line_number, current_scope,
...
std::shared_ptr<ast::Node> ASTGenerator::create_if(
    std::shared_ptr<ast::Node> conditions,
    std::vector<std::shared_ptr<ast::Node>> if_body,
    std::vector<std::shared_ptr<ast::Node>> else_body) {
    auto if_node = std::shared_ptr<ast::IfNode>(
        new ast::IfNode(line_number, current_scope, ir_generators.if_generator,
                        conditions, if_body, else_body));
    return if_node;
}
std::shared_ptr<ast::Node> ASTGenerator::create_for(
    std::shared_ptr<ast::Node> left, std::shared_ptr<ast::Node> center,
    std::shared_ptr<ast::Node> right,
    std::vector<std::shared_ptr<ast::Node>> body) {
    auto for_node = std::shared_ptr<ast::ForNode>(new ast::ForNode(
        line_number, current_scope, ir_generators.for_generator, left, center,
        right, body));
    return for_node;
}

std::shared_ptr<ast::AccessNode> ASTGenerator::create_access(
    std::string left, std::string right) {
    auto left_node = get_named_value(left);
    auto accessing = std::shared_ptr<ast::AccessNode>(new ast::AccessNode(
...
std::shared_ptr<ast::ArrayNode> ASTGenerator::create_list(
    std::vector<std::shared_ptr<ast::Node>> elements) {
    auto list_node = std::shared_ptr<ast::ArrayNode>(new ast::ArrayNode(
...
std::shared_ptr<ast::Node> ASTGenerator::create_block_end() {
    auto end_scope = variable_collector.get_namespace();
    auto node = std::shared_ptr<ast::BlockEndNode>(
        new ast::BlockEndNode(line_number, current_scope,
