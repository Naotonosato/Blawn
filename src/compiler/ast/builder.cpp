...
std::shared_ptr<VariableNode> Builder::create_variable_definition(
    ...
    node_collector.add(name, Node::create(variable));
    return variable;
}
...
std::shared_ptr<GlobalVariableNode> Builder::create_global_variable_definition(
    ...
    node_collector.add(name, Node::create(variable), utils::Scope("[GLOBAL]"));
    return variable;
}
...
std::shared_ptr<ArgumentNode> Builder::create_argument(
    ...
    node_collector.add(name, Node::create(argument));
    return argument;
}
...
std::shared_ptr<CallFunctionNode> Builder::create_call(
    ...
    auto got_node = *node_collector.get(function_name);
    if (got_node) {
        if (got_node->is_type<GenericFunctionDeclarationNode>()) {
            auto declaration =
                got_node->get_pointer<GenericFunctionDeclarationNode>();
            return std::make_shared<CallFunctionNode>(
                current_line_number, declaration, std::move(arguments));
        }
    }
    std::cout << "err no function " << function_name << std::endl;
    return nullptr;
}
...
Builder::create_generic_function_declaration(
    ...
    auto declaration = std::make_shared<GenericFunctionDeclarationNode>(
        current_line_number, name, arguments.size());
    node_collector.add(name, Node::create(declaration));
    return declaration;
}
...
Builder::create_generic_function_definition(
    ...
    auto declaration = (*node_collector.get(name))
...
std::shared_ptr<Node> Builder::create_generic_class_declaration(
    ...
    node_collector.add(name, declaration);
    return declaration;
}
*/
...
std::shared_ptr<GenericClassNode> Builder::create_generic_class_definition(
    ...
    node_collector.add(name, Node::create(definition));
    return definition;
}
...
std::shared_ptr<RootNode> Builder::get_root_node() const { return root; }

utils::ScopedCollector<std::shared_ptr<Node>>& Builder::get_node_collector() {
