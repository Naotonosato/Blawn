...
class Scope {
    private:
    std::vector<std::string> scope_name_vector;
    public:
    std::string& operator[](size_t index)
    {
        return scope_name_vector[index];
    }
    ...
    size_t size() const
    {
        return scope_name_vector.size();
    }

    auto begin()
    {
        return scope_name_vector.begin();
    }

    auto end()
    {
        return scope_name_vector.end();
    }

    void push_back(const std::string name)
    {
        scope_name_vector.push_back(name);
    }
    void pop_back()
    {
        scope_name_vector.pop_back();   
    }

    bool is_longer(Scope& s) {
        if (s.size() <= this->size()) return false;
        unsigned int index = 0;
        for (auto& e : scope_name_vector) {
...
class ScopedCollector {
    ...
    public:
    ScopedCollector(std::string top = "[TOP]")
    ...
    const Scope& get_namespace() { return current_namespace; }
