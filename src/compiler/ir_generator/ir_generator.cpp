...
void StoreIRGenerator::walk(
    ...
    ast::DeepCopyNode& node, llvm::LLVMContext& context, llvm::Module& module,
        ...
        for (auto& element : field) {
            ...
            if (p_node->generate()->getType()->isPointerTy() &&
                p_node->generate()
                    ...
                    ->isStructTy()) {
                auto store_node =
                    ast::DeepCopyNode(node.line_number, node.self_scope,
                ...
                if (p_node->is_accessing()) {
                    llvm::Value* element_pointer =
                        static_cast<ast::AccessNode*>(p_node.get())
                            ->get_pointer();
                    res.push_back(
...
void StoreIRGenerator::store(ast::DeepCopyNode& node, llvm::Value* pointer,
...
llvm::Value* StoreIRGenerator::generate(ast::Node& node_) {
    ...
    if (node.pointer->is_accessing() && !pointer->getType()->isStructTy()) {
        pointer =
            (*static_cast<ast::AccessNode*>(node.pointer.get())).get_pointer();
        bool is_break = false;
...
llvm::Value* CallFunctionIRGenerator::generate(ast::Node& node_) {
    ...
    if (node.is_builtin) {
        for (auto& arg : node.passed_arguments) {
            if (arg->is_argument()) {
                if (static_cast<ast::ArgumentNode*>(arg.get())
                        ->get_right_value() == nullptr) {
    ...
    for (auto& arg : node.passed_arguments) {
        if (arg->is_argument()) {
            if (static_cast<ast::ArgumentNode*>(arg.get())->get_right_value() ==
                nullptr) {
                std::vector<llvm::Value*> empty_arg;
...
llvm::Value* AccessIRGenerator::generate(ast::Node& node_) {
    ...
    if (index != -1) {
        ...
        if (bm != nullptr) {
            ...
            auto method_call = ast::CallFunctionNode(
