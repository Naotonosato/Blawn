...
void initialize(llvm::LLVMContext& context, llvm::Module& module,
  ...
  auto malloc_declaration_type =
      llvm::FunctionType::get(ir_builder.getInt64Ty()->getPointerTo(), malloc_types, false);
  llvm::Function::Create(malloc_declaration_type, llvm::Function::ExternalLinkage, "malloc",
                         &module);
  // declare free
  ...
  llvm::Function::Create(free_declaration_type, llvm::Function::ExternalLinkage, "free",
                         &module);
  ...
  auto function_type =
      llvm::FunctionType::get(llvm::Type::getInt8Ty(context), main_types, false);
  auto function =
...
llvm::Value* NullIRGenerator::generate(Node& node_) {
  ...
  std::vector<std::string> unsigned_types = {"__C_UCHAR__", "__C_UINT__", "__C_ULONG__",
                                             "__C_ULONGLONG__", "__C_USHORT__"};

  std::vector<std::string> builtin_C_types = {
      "__C_BOOL__",   "__C_CHAR__",  "__C_CHAR__",       "__C_DOUBLE__",   "__C_FLOAT__",
      "__C_INT__",    "__C_LONG__",  "__C_LONGDOUBLE__", "__C_LONGLONG__", "__C_SCHAR__",
      "__C_SHORT__",  "__C_UCHAR__", "__C_UINT__",       "__C_ULONG__",    "__C_ULONGLONG__",
      "__C_USHORT__", "__C_WCHAR__", "__C_UNKNOWN__"};
  ...
  if ((res.back() == "" or res.size() <= 1) && node.class_node == nullptr) {
    logger.unknown_identifier_error("type name or variable", node.type_name);
  }
  parsed.pop_back();
  ...
  if (node.class_node == nullptr) {
    ...
    if (utils::exist(builtin_C_types, type_name)) {
      if (parsed.size() == 0) {
        if (!is_real_num_type)
          return llvm::ConstantInt::get(context, llvm::APInt(type_size, 0, is_unsigned));
        return llvm::ConstantFP::get(context, llvm::APFloat(0.0));
      }
      llvm::Type* type =
          is_real_num_type ? ir_builder.getFloatTy() : ir_builder.getIntNTy(type_size);
      for (auto& _ : parsed) {
    ...
    auto type = utils::get_or_create_type(context, node.class_node, node.class_node->name);
    auto constructor_type = llvm::FunctionType::get(type->getPointerTo(), false);
    auto constructor = llvm::Function::Create(constructor_type, llvm::Function::ExternalLinkage,
                                              node.class_node->name, &module);
    auto constructor_entry = llvm::BasicBlock::Create(context, "entry", constructor);
    auto previous_block    = ir_builder.GetInsertBlock();
    ir_builder.SetInsertPoint(constructor_entry);
    ir_builder.CreateRet(utils::malloc_value(type, context, module, ir_builder));
...
llvm::Value* StringIRGenerator::generate(Node& node) {
  ...
  auto string = ir_builder.CreateCall(module.getFunction("string_constructor"), args);
  return string;
}
  ...
  if (!node.is_generated()) {
    ...
    if (node.is_global()) {
      auto gvalue = new llvm::GlobalVariable(
          module, right->getType(), false, llvm::GlobalVariable::ExternalLinkage,
...
llvm::Value* AssigmentIRGenerator::generate(Node& node_) {
  ...
  if (right_value->getType()->isPointerTy() &&
    ...
    logger.assignment_different_scope_error(utils::to_string(left_node->self_scope),
  ...
  if (right_value->getType() == ir_builder.getInt8PtrTy()) {
    ...
    to_store =
        ir_builder.CreateBitCast(right_value, pointer->getType()->getPointerElementType());
  }
  if (pointer->getType()->getPointerElementType() != to_store->getType()) {
    BlawnLogger logger;
    logger.set_line_number(node.line_number);
    logger.different_type_error(utils::to_string(pointer->getType()->getPointerElementType()),
...
void StoreIRGenerator::walk(llvm::Type* type,
  ...
  if (type->isPointerTy() && type->getPointerElementType()->isStructTy()) {
    ...
    for (auto& element : field) {
      ...
      p_node = std::shared_ptr<Node>(new AccessNode(node.line_number, node.self_scope, gen,
                                                    node.pointer, element_name, empty));
      o_node = std::shared_ptr<Node>(new AccessNode(node.line_number, node.self_scope, gen,
                                                    node.object, element_name, empty));
      if (p_node->generate()->getType()->isPointerTy() &&
          p_node->generate()->getType()->getPointerElementType()->isStructTy()) {
        auto store_node =
            StoreNode(node.line_number, node.self_scope, store_generator, p_node, o_node);
        walk(p_node->generate()->getType(), res, store_node, context, module, ir_builder,
             store_generator);
      } else {
        p_node->generate();
        ...
        if (p_node->is_accessing()) {
          ...
          res.push_back(std::make_pair(element_pointer, element_object));
        }
      }
    }
  }
...
llvm::Value* StoreIRGenerator::generate(Node& node_) {
  ...
  if (node.pointer->is_accessing() && !pointer->getType()->isStructTy()) {
    ...
    if (pointer->getType()->getPointerElementType()->isPointerTy()) {
      pointer  = node.pointer->generate(); // ir_builder.CreateLoad(pointer);
      is_break = true;
    }
    if (!pointer->getType()->getPointerElementType()->isPointerTy() &&
...
llvm::Function* FunctionIRGenerator::generate(Node& node_) {
  ...
  auto func_type = llvm::FunctionType::get(llvm::Type::getVoidTy(context), types, false);
  auto function =
      llvm::Function::Create(func_type, llvm::Function::ExternalLinkage, node.name, &module);
...
llvm::Value* DeclareCIRGenerator::generate(Node& node_) {
  ...
  for (auto& v : node.arguments_type()) {
    ...
    if (value == nullptr) {
      logger.invalid_paramater_error(node.name + "(invalide declaration of argument)");
    }
...
llvm::Value* CallFunctionIRGenerator::generate(Node& node_) {
  ...
  auto base_function      = llvm::Function::Create(
      base_function_type, llvm::Function::ExternalLinkage, node.function->name, &module);
  ...
  if (node.function->arguments_names.size()) {
    ...
    for (auto& tmp_func_arg : base_function->args()) {
      ...
      empty_node->alloca_inst = ir_builder.CreateAlloca(tmp_func_arg.getType(), 0, 0);
  ...
  if (func_end != nullptr) func_end->generate();
  if (return_value != nullptr)
  ...
  auto new_function = llvm::Function::Create(new_function_type, llvm::Function::ExternalLinkage,
...
llvm::Value* ClassIRGenerator::generate(Node& node_) {
  ...
  auto constructor =
      llvm::Function::Create(type, llvm::Function::ExternalLinkage, node.name, &module);
...
llvm::Value* CallConstructorIRGenerator::generate(Node& node_) {
  ...
  auto base_constructor      = llvm::Function::Create(
      base_constructor_type, llvm::Function::ExternalLinkage, node.get_class()->name, &module);
  ...
  for (auto& name : node.get_class()->get_arguments_names()) {
    ...
    auto local              = node.get_class()->get_self_namespace();
    auto empty_node         = node.argument_collector.get(name, local);
    empty_node->alloca_inst = ir_builder.CreateAlloca(tmp_constructor_arg->getType(), 0, 0);
    ir_builder.CreateStore(tmp_constructor_arg, empty_node->alloca_inst);
    empty_node->set_right_value(tmp_constructor_arg);
  ...
  auto new_constructor = llvm::Function::Create(
      new_constructor_type, llvm::Function::ExternalLinkage, node.get_class()->name, &module);
  ...
  if (destructor == nullptr) {
    auto destructor_type =
        llvm::FunctionType::get(ir_builder.getVoidTy(), destructor_args, false);
    destructor = llvm::Function::Create(destructor_type, llvm::Function::ExternalLinkage,
  ...
  if (cond_value->getType() != ir_builder.getInt1Ty()) {
    ...
    if (cond_value->getType()->isFloatTy())
      cond_value = ir_builder.CreateICmpNE(
          cond_value, llvm::ConstantFP::get(context, llvm::APFloat((0.0))));
...
llvm::Value* AccessIRGenerator::generate(Node& node_) {
  ...
  if (left_type->isPointerTy()) {
    struct_type = llvm::dyn_cast<llvm::StructType>(left_type->getPointerElementType());
  ...
  if (index != -1) {
    ...
    if (bm != nullptr) {
      ...
      auto method_call =
          CallFunctionNode(node.line_number, node.self_scope, gen, call_node->passed_arguments,
