...
std::shared_ptr<Node> ASTGenerator::create_assign(std::string name,
  ...
  if (variable_collector.exist(name)) {
    ...
    auto assigment = std::shared_ptr<AssigmentNode>(
        new AssigmentNode(line_number, current_scope, ir_generators.assigment_generator,
  ...
  if (argument_collector.exist(name)) {
    ...
    auto assigment = std::shared_ptr<AssigmentNode>(new AssigmentNode(
        line_number, current_scope, ir_generators.assigment_generator, right_node, arg, name));
    return assigment;
  } else {
    auto variable = std::shared_ptr<VariableNode>(
        new VariableNode(line_number, current_scope, ir_generators.variable_generator,
...
std::shared_ptr<Node> ASTGenerator::create_assign(std::shared_ptr<AccessNode> left,
                                                  std::shared_ptr<Node> right) {
  auto assigment = std::shared_ptr<AssigmentNode>(new AssigmentNode(
      line_number, current_scope, ir_generators.assigment_generator, right, left));
  return assigment;
}

std::shared_ptr<Node> ASTGenerator::create_store(std::shared_ptr<Node> pointer,
                                                 std::shared_ptr<Node> object) {
  auto node = std::shared_ptr<StoreNode>(new StoreNode(
      line_number, current_scope, ir_generators.store_generator, pointer, object));
  return node;
}

std::shared_ptr<Node> ASTGenerator::declare_C_function(std::string name,
                                                       std::vector<std::shared_ptr<Node>> args,
                                                       std::shared_ptr<Node> return_) {
  std::unique_ptr<DeclareCNode> node(new DeclareCNode(
      line_number, current_scope, ir_generators.declare_C_generator, name, args, return_));
...
std::shared_ptr<Node> ASTGenerator::create_C_member(std::string name) {
  ...
  auto node = std::shared_ptr<Node>(
      new NullNode(line_number, current_scope, ir_generators.null_generator, name, class_node));
  return node;
}
...
void ASTGenerator::add_argument(std::string arg_name) {
  ...
  arg_scope.pop_back();
  auto argument = std::shared_ptr<ArgumentNode>(
      new ArgumentNode(line_number, arg_scope, ir_generators.argument_generator, arg_name));
...
std::shared_ptr<FunctionNode> ASTGenerator::add_function(
    ...
    std::vector<std::shared_ptr<Node>> body, std::shared_ptr<Node> return_value) {
  if (function_collector.exist(name)) {
    logger.set_line_number(line_number);
    logger.multiple_definition_error("function", name);
  }
  auto func = std::shared_ptr<FunctionNode>(
      new FunctionNode(line_number, current_scope, ir_generators.function_generator, name,
...
std::shared_ptr<ClassNode> ASTGenerator::create_class(
  ...
  auto class_ = std::shared_ptr<ClassNode>(
      new ClassNode(line_number, current_scope, ir_generators.class_generator, methods,
...
std::shared_ptr<ClassNode> ASTGenerator::create_C_type(
  ...
  */
  auto c_type          = std::shared_ptr<ClassNode>(new ClassNode(line_number, current_scope,
...
std::unique_ptr<Node> ASTGenerator::create_call(std::string name,
  ...
  if (get_blawn_context().exist_builtin_function(name)) {
    ...
    auto calling = std::unique_ptr<CallFunctionNode>(
        new CallFunctionNode(line_number, current_scope, ir_generators.calling_generator,
  ...
  if (class_collector.exist(name)) {
    ...
    auto constructor = std::unique_ptr<CallConstructorNode>(new CallConstructorNode(
        line_number, current_scope, ir_generators.call_constructor_generator, class_, arguments,
  ...
  if (C_type_collector.exist(name)) {
    ...
    auto constructor = std::unique_ptr<CallConstructorNode>(new CallConstructorNode(
        line_number, current_scope, ir_generators.call_constructor_generator, C_type, arguments,
        class_collector.get_namespace(), argument_collector));
    return std::move(constructor);
  } else {
    auto function = function_collector.get(name);
...
std::shared_ptr<Node> ASTGenerator::create_call(std::shared_ptr<AccessNode> left,
                                                std::vector<std::shared_ptr<Node>> arguments) {
  arguments.insert(arguments.begin(), left->get_left_node());
  auto call_node = std::shared_ptr<CallFunctionNode>(
      new CallFunctionNode(line_number, current_scope, ir_generators.calling_generator, nullptr,
...
std::unique_ptr<IntegerNode> ASTGenerator::create_integer(int num) {
  auto Integer = std::unique_ptr<IntegerNode>(
      new IntegerNode(line_number, current_scope, ir_generators.int_ir_generator));
  Integer->int_num = num;
...
std::unique_ptr<FloatNode> ASTGenerator::create_float(double num) {
  auto float_ = std::unique_ptr<FloatNode>(
      new FloatNode(line_number, current_scope, ir_generators.float_ir_generator));
...
std::shared_ptr<StringNode> ASTGenerator::create_string(std::string str) {
  auto string =
      std::make_shared<StringNode>(line_number, current_scope, ir_generators.string_generator);
  string->string = str;
  return string;
}
...
std::unique_ptr<BinaryExpressionNode> ASTGenerator::attach_operator(
  ...
  auto expression           = std::unique_ptr<BinaryExpressionNode>(new BinaryExpressionNode(
      line_number, current_scope, ir_generators.binary_expression_generator));
...
std::shared_ptr<Node> ASTGenerator::create_if(std::shared_ptr<Node> conditions,
  ...
  auto if_node = std::shared_ptr<IfNode>(new IfNode(
      line_number, current_scope, ir_generators.if_generator, conditions, if_body, else_body));
  return if_node;
}
std::shared_ptr<Node> ASTGenerator::create_for(std::shared_ptr<Node> left,
                                               std::shared_ptr<Node> center,
                                               std::shared_ptr<Node> right,
                                               std::vector<std::shared_ptr<Node>> body) {
  auto for_node = std::shared_ptr<ForNode>(new ForNode(
      line_number, current_scope, ir_generators.for_generator, left, center, right, body));
  return for_node;
}
...
std::shared_ptr<AccessNode> ASTGenerator::create_access(std::string left, std::string right) {
  ...
  auto accessing = std::shared_ptr<AccessNode>(
      new AccessNode(line_number, left_node->self_scope, ir_generators.access_generator,
                     left_node, right, function_collector));
  return accessing;
}

std::shared_ptr<AccessNode> ASTGenerator::create_access(std::shared_ptr<Node> left,
                                                        std::string right) {
  auto accessing = std::shared_ptr<AccessNode>(new AccessNode(line_number, left->self_scope,
                                                              ir_generators.access_generator,
                                                              left, right, function_collector));
  return accessing;
}
...
std::shared_ptr<ListNode> ASTGenerator::create_list(
  ...
  auto list_node = std::shared_ptr<ListNode>(
      new ListNode(line_number, current_scope, ir_generators.list_generator, elements));
  return list_node;
}
std::shared_ptr<ListNode> ASTGenerator::create_list() {
  auto list_node = std::shared_ptr<ListNode>(
      new ListNode(line_number, current_scope, ir_generators.list_generator, true));
  return list_node;
}
...
std::shared_ptr<Node> ASTGenerator::create_block_end() {
  ...
  auto node      = std::shared_ptr<BlockEndNode>(new BlockEndNode(
      line_number, current_scope, ir_generators.block_end_generator, end_scope));
  return node;
}