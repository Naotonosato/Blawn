...
std::vector<std::string> get_compile_commands(int argc, char** argv) {
  ...
  std::string cmd1 = "llc -O3 " + ir_filename + " -o " + abs("tmp/result.s");
  commands.push_back(cmd1.c_str());
  if (is_link) {
    commands.push_back("gcc -c " + abs("tmp/result.s") + " -o " + abs("tmp/obj.o") +
                       " -no-pie");
    std::string cmd = ("gcc -no-pie " + abs("tmp/obj.o") + " " + abs("data/builtins.o") + " " +
                       to_link + " -o " + output_filename + " " + arguments);
    commands.push_back(cmd.c_str());
  } else {
    std::string cmd = "gcc " + arguments + " " + abs("tmp/result.s") + " " +
                      abs("data/builtins.o") + " -no-pie -o " + output_filename;
    commands.push_back(cmd.c_str());
  }
...
int compile(int argc, char** argv) {
  ...
  llvm::raw_fd_ostream stream(output_filename.c_str(), error, llvm::sys::fs::OpenFlags::F_None);
