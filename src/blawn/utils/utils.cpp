...
void utils::split(std::string text, std::string splitter, std::vector<std::string>& res) {
...
llvm::Value* utils::free_value(llvm::Value* to_delete_ptr, llvm::Module& module,
  ...
  auto cast_inst =
      ir_builder.CreateBitCast(to_delete_ptr, ir_builder.getInt64Ty()->getPointerTo());
...
llvm::Type* utils::get_or_create_type(llvm::LLVMContext& context,
                                      std::shared_ptr<ClassNode> class_node, std::string name) {
  ...
  if (type == nullptr) {
    ...
    for (auto& name : names) {
      get_blawn_context().register_element_name(type->getStructName(), name, index);
      index += 1;
    }
