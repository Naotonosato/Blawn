...
#include "include/blawn/converter/ast_to_mir/converter.hpp"
...
std::unique_ptr<mir::MIR> Converter::visit(
    ...
    std::cout << typeid(node).name() << std::endl;
    std::cout << "declaration of calling function is "
              << node->get_function()->get_name() << "with arguments count "
              << node->get_function()->get_num_arguments() << std::endl;
    return 0;
}
std::unique_ptr<mir::MIR> Converter::visit(
...
std::unique_ptr<mir::MIR> Converter::visit(std::shared_ptr<ast::IfNode> node) {
    std::cout << typeid(node).name() << std::endl;
    return 0;
}
std::unique_ptr<mir::MIR> Converter::visit(std::shared_ptr<ast::ForNode> node) {
    std::cout << typeid(node).name() << std::endl;
    return 0;
}
std::unique_ptr<mir::MIR> Converter::visit(
