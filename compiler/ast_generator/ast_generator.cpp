...
std::shared_ptr<Node> ASTGenerator::create_assign(
    ...
    if (argument_collector.exist(name)) {
        ...
        if (is_global) variable_collector.set(name, variable,{"TOP"});
        else variable_collector.set(name, variable);
        return variable;
    }
}
...
std::shared_ptr<Node> ASTGenerator::create_C_member(std::string name) {
    ...
    auto node = std::shared_ptr<Node>(new NullNode(
        line_number,current_scope, ir_generators.null_generator, name, class_node));
    return node;
}
...
std::shared_ptr<ClassNode> ASTGenerator::create_C_type(
    ...
    */
    ...
    C_type_collector.set(name, c_type, previous_scope);
    return c_type;
}
...
std::unique_ptr<Node> ASTGenerator::create_call(
    ...
    if (name == "__blawn_cast__" && arguments.size() == 2) {
        auto cast_node = std::unique_ptr<CastNode>(
            new CastNode(line_number,current_scope, ir_generators.cast_generator,
    ...
    if (class_collector.exist(name)) {
        ...
        auto constructor =
            std::unique_ptr<CallConstructorNode>(new CallConstructorNode(
                line_number,current_scope, ir_generators.call_constructor_generator, class_,
    ...
    if (C_type_collector.exist(name)) {
        ...
        auto constructor =
            std::unique_ptr<CallConstructorNode>(new CallConstructorNode(
                line_number,current_scope, ir_generators.call_constructor_generator, C_type,
                arguments, class_collector.get_namespace(),
                argument_collector));
        return std::move(constructor);
    } else {
...
std::unique_ptr<StringNode> ASTGenerator::create_string(std::string str) {
    auto string = std::make_unique<StringNode>(line_number,current_scope,
...
std::shared_ptr<Node> ASTGenerator::create_if(
    ...
    auto if_node = std::shared_ptr<IfNode>(new IfNode(
        line_number,current_scope, ir_generators.if_generator, conditions, body, empty));
...
std::shared_ptr<Node> ASTGenerator::add_else(
    ...
    auto res = std::make_shared<Node>(line_number,current_scope, ir_generators.ir_generator);
    return res;
}
...
std::shared_ptr<AccessNode> ASTGenerator::create_access(std::string left,
    ...
    auto accessing = std::shared_ptr<AccessNode>(
        new AccessNode(line_number,left_node->self_scope, ir_generators.access_generator, left_node,
...
std::shared_ptr<AccessNode> ASTGenerator::create_access(
    ...
    auto accessing = std::shared_ptr<AccessNode>(
        new AccessNode(line_number,left->self_scope, ir_generators.access_generator, left, right,
                       function_collector));
    return accessing;
}
